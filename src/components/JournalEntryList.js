        ...
        {entries.map((entry, idx) => (
          ...
          >
            <div className="flex flex-wrap gap-2 text-base font-semibold mb-2">
              ...
              <div><span className="text-gray-400">{fieldLabel.session}:</span> <span>{entry.session}</span></div>
              <div><span className="text-gray-400">{fieldLabel.liquiditySweep}:</span> <span>{entry.liquiditySweep}</span></div>
            </div>
            <div className="flex flex-wrap gap-2 text-lg font-bold mb-2">
              <div><span className="text-gray-400">{fieldLabel.pnl}:</span> <span className={entry.pnl > 0 ? "text-green-400" : entry.pnl < 0 ? "text-red-400" : "text-gray-100"}>{entry.pnl > 0 ? "+" : ""}{entry.pnl}</span></div>
              <div><span className="text-gray-400">{fieldLabel.rr}:</span> <span className="text-blue-300">{entry.rr}</span></div>
              <div><span className="text-gray-400">{fieldLabel.accountBalance}:</span> <span>{entry.accountBalance}</span></div>
              <div><span className="text-gray-400">{fieldLabel.duration}:</span> <span>{entry.duration}</span></div>
            </div>
            <div className="flex flex-wrap gap-2 text-xs mb-1">
              <div><span className="text-gray-400">{fieldLabel.entryTime}:</span> <span>{entry.entryTime}</span></div>
              <div><span className="text-gray-400">{fieldLabel.exitTime}:</span> <span>{entry.exitTime}</span></div>
            </div>
            <div className="text-xs text-neutral-500 mb-1">{new Date(entry.created).toLocaleString()}</div>
